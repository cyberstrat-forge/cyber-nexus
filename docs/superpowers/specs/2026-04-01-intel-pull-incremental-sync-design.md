...
文件名格式 `{YYYYMMDD}-{item_id}.md`，同一 `item_id` 覆盖写入。

### API 请求幂等

`since` 使用 `>=` 比较，可能重复获取边界数据，通过文件覆盖实现去重。
...
| 旧参数 | 新参数 | 说明 |
|--------|--------|------|
| `from=beginning` | `since=beginning` | 全量同步起点 |
| `since={datetime}`（时间范围起点） | 不支持 | API 不再支持时间范围过滤 |
| `until={datetime}`（时间范围终点） | 不支持 | API 不再支持时间范围过滤 |

### 状态文件迁移

旧版本状态文件需重新执行 `--init` 全量同步。
