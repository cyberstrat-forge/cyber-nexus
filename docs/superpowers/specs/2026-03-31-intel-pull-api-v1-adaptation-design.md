...
| 状态码 | 说明 | 处理方式 |
|--------|------|---------|
| 400 | 参数错误（cursor 和 from 同时使用、cursor 格式无效） | 记录错误，提示用户检查参数 |
| 401 | API Key 无效或权限不足 | 提示用户检查 API Key 配置 |
| 404 | cursor 指定的 item 不存在 | 清空 cursor，提示用户执行 `--init` |
| 500 | 服务器内部错误 | 记录错误，建议稍后重试 |
| 503 | 服务不可用 | 记录错误，建议稍后重试 |

### intel-pull 错误处理

| 错误场景 | 错误码 | 处理方式 |
|----------|--------|---------|
| 配置文件不存在 | `CONFIG_NOT_FOUND` | 显示配置示例和创建步骤 |
| 配置文件格式错误 | `CONFIG_PARSE_ERROR` | 显示具体错误位置 |
| 源名称不存在 | `SOURCE_NOT_FOUND` | 显示可用源列表 |
| API Key 未配置 | `API_KEY_NOT_SET` | 提示用户配置 API Key |
| API 连接失败 | `API_CONNECTION_FAILED` | 建议检查网络和 URL |
| API 认证失败 | `API_AUTH_FAILED` | 提示检查 API Key |
| API 请求超时 | `API_TIMEOUT` | 自动重试 1 次，仍失败则记录错误 |
| API 返回无效响应 | `API_INVALID_RESPONSE` | 记录响应内容，便于调试 |
| 参数组合冲突 | `INVALID_OPTIONS` | 显示参数使用说明 |

### 预处理脚本错误处理

| 错误场景 | 错误码 | 处理方式 |
|----------|--------|---------|
| cyber-pulse 文件缺少必要字段 | `INVALID_PULSE_FORMAT` | 生成 `.error.md`，保留在 inbox/ |
| 文件写入失败 | `WRITE_FAILED` | 记录错误，跳过该文件 |
| 目录创建失败 | `DIR_CREATE_FAILED` | 记录错误，提示权限问题 |
...
**文件名**: 20260330-item_a1b2c3d4.md
**处理时间**: 2026-03-30 10:00:00
**错误码**: INVALID_PULSE_FORMAT
...
| 场景 | 重试次数 | 重试间隔 | 说明 |
|------|---------|---------|------|
| API 请求超时 | 1 次 | 2 秒 | 超时后自动重试 |
| API 连接失败 | 0 次 | - | 不重试，直接报错 |
| API 500/503 错误 | 0 次 | - | 不重试，建议稍后重试 |
