...
**变更**
- WikiLink 别名格式：所有 WikiLink 输出使用 `[[path|alias]]` 格式，Obsidian 显示更友好

**测试**
- 新增 7 个 WikiLink 相关测试用例
...
**优化**
- 报告工作流上下文优化：`scan-cards.ts` 新增 `--format agent` 输出格式，保护主会话上下文空间
- 代码重构：提取 `scanCardsBase<T>()` 泛型函数，消除重复代码

**修复**
- Obsidian 链接格式：统一使用 `[[intelligence/{完整路径}|标题]]` 格式
- Agent 临时文件管理：为三个 Agent 添加临时文件管理指导，防止残留临时文件
...
**新增**
- 四层情报报告体系：日报（信号）→ 周报（主题）→ 月报（变化）→ 年报（趋势）
- 变化类型系统：6 种变化类型支撑周报主题标注和月报聚合
- 态势追踪机制：态势 ID（S-YYYY-MM-XX）支持跨月追踪
- 新增组件：
  - 类型定义：`scripts/reporting/types/`
  - 报告扫描脚本：`scan-reports.ts`
  - 周报/月报/年报 Agent 和模板

**变更**
- intel-distill 命令：区分四种报告类型的数据源和 Agent 调用
- intelligence-briefing-writer Agent：标记为 legacy
...
**变更**
- Obsidian Bases 索引改为手工维护：移除自动生成步骤，已存在的 `_index.base` 文件不受影响
